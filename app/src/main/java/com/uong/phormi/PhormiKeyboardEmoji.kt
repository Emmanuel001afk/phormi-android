...
object PhormiKeyboardEmoji {
    ...
    private fun unicodeEmojiCodePoints(): List<Int> {
        ...
        for (cp in 0..0x10FFFF) {
            if (UCharacter.hasBinaryProperty(cp, UProperty.EMOJI) && cp !in 0x1F1E6..0x1F1FF) {
                result.add(cp)
            }
        }
        return result
    ...
    private fun category(start: Int, end: Int): List<String> =
        all.filter { it.codePointAt(0) in start..end }
    ...
    val categories: LinkedHashMap<String, List<String>> by lazy {
        linkedMapOf(
            ...
            "🔣" to all.filter {
                ...
                cp in 0x2000..0x2BFF || cp in 0x1F100..0x1F2FF
            },
            "🇳🇬" to countryFlags()
        )
    }
