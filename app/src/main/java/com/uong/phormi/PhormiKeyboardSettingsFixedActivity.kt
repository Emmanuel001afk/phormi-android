...
class PhormiKeyboardSettingsFixedActivity : Activity() {
    ...
    private fun switchRow(text:String,checked:Boolean,on:(Boolean)->Unit)=Switch(this).apply{this.text=text;this.isChecked=checked;setTextColor(Color.WHITE);setPadding(dp(4),dp(8),dp(4),dp(8));setOnCheckedChangeListener{_,v->on(v)}}
    ...
    private fun render(){
        ...
        root.addView(switchRow("Suggestions from the first character",PhormiKeyboardPreferences.suggestions(this)){v->PhormiKeyboardPreferences.set(this@PhormiKeyboardSettingsFixedActivity,PhormiKeyboardPreferences.KEY_SUGGESTIONS,v)})
        root.addView(switchRow("Autocorrect",PhormiKeyboardPreferences.autocorrect(this)){v->PhormiKeyboardPreferences.set(this@PhormiKeyboardSettingsFixedActivity,PhormiKeyboardPreferences.KEY_AUTOCORRECT,v)})
        root.addView(switchRow("Automatic capitalization",PhormiKeyboardPreferences.autoCaps(this)){v->PhormiKeyboardPreferences.set(this@PhormiKeyboardSettingsFixedActivity,PhormiKeyboardPreferences.KEY_AUTO_CAPS,v)})
        root.addView(switchRow("Haptic key feedback",PhormiKeyboardPreferences.haptic(this)){v->PhormiKeyboardPreferences.set(this@PhormiKeyboardSettingsFixedActivity,PhormiKeyboardPreferences.KEY_HAPTIC,v)})
        root.addView(switchRow("Key sounds",PhormiKeyboardPreferences.sound(this)){v->PhormiKeyboardPreferences.set(this@PhormiKeyboardSettingsFixedActivity,PhormiKeyboardPreferences.KEY_SOUND,v)})
        root.addView(switchRow("Automatic contextual AI emoji",PhormiKeyboardPreferences.aiEmoji(this)){v->PhormiKeyboardPreferences.set(this@PhormiKeyboardSettingsFixedActivity,PhormiKeyboardPreferences.KEY_AI_EMOJI,v)})
        root.addView(switchRow("Floating keyboard appearance",getSharedPreferences("phormi_keyboard_fixed",MODE_PRIVATE).getBoolean("floating",false)){v->getSharedPreferences("phormi_keyboard_fixed",MODE_PRIVATE).edit().putBoolean("floating",v).apply()})
