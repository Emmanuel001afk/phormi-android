...
/** Real tab-group manager: groups persist by tab ID and selecting a tab reaches the live browser. */
class TabGroupsActivity : AppCompatActivity() {
    ...
    private fun render() {
        ...
        if (groups.isEmpty()) {
            list.addView(TextView(this).apply { text = "No groups yet. Create a group, then assign tabs from Tab overview."; setTextColor(0xFF94A3B8.toInt()); setPadding(18, 30, 18, 30) }); return
        }
        val tabData = readCurrentTabs()
        groups.forEach { group ->
            val row = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(16, 14, 16, 14); setBackgroundColor(0xFF172033.toInt()) }
            row.addView(TextView(this).apply { text = group.name; textSize = 17f; setTextColor(0xFFF8FAFC.toInt()) })
            row.addView(TextView(this).apply { text = "${group.tabIds.size} tabs${if (group.taskNote.isNotBlank()) " · ${group.taskNote}" else ""}"; textSize = 12f; setTextColor(0xFF94A3B8.toInt()); setPadding(0, 4, 0, 8) })
    ...
    private fun readCurrentTabs(): List<Triple<Int, String, String>> {
        ...
        return buildList { for (i in 0 until ids.length()) { val id = ids.optInt(i, 0); if (id > 0) add(Triple(id, titles.optString(i, "Tab $id").ifBlank { "Tab $id" }, urls.optString(i, "about:blank"))) } }
    }

    private fun createGroup() {
        val input = EditText(this).apply { hint = "Group name"; setSingleLine(true) }
        AlertDialog.Builder(this).setTitle("New tab group").setView(input).setNegativeButton("Cancel", null).setPositiveButton("Create") { _, _ -> TabGroupManager(this).create(input.text.toString()); render() }.show()
    }
    ...
    private fun editGroup(id: String) {
        ...
        box.addView(name); box.addView(note)
        AlertDialog.Builder(this).setTitle("Edit group").setView(box)
            .setNegativeButton("Delete") { _, _ -> TabGroupManager(this).delete(id); render() }
            .setPositiveButton("Save") { _, _ -> TabGroupManager(this).rename(id, name.text.toString()); TabGroupManager(this).setTaskNote(id, note.text.toString()); render() }.show()
    }
}
