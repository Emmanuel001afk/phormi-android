...
object PhormiKeyboardTextEngine {
    ...
    private val correctionsByLanguage = mapOf(
        ...
        "ig" to mapOf("daalu" to "daalụ", "biko" to "biko", "ee" to "ee", "mba" to "mba"),
    ...
    /** Prefer the selected Phormi IME subtype locale; fall back to EditorInfo hints and device locale. */
    fun localeFor(info: EditorInfo?): Locale = runCatching {
        ...
        val subtype = service?.currentInputMethodSubtype
    ...
    fun suggestions(context: Context, prefix: String, locale: Locale = activeLocale(context)): List<String> {
        val p = prefix.trim().lowercase(locale)
    ...
    fun learnPair(context: Context, previous: String, current: String, info: EditorInfo?) {
        ...
        val key = previous.lowercase(localeFor(info)); val map = loadAllNextWords(context).toMutableMap(); map[key] = (map[key].orEmpty().filterNot { it.equals(current, true) } + current.take(64)).takeLast(10); saveAllNextWords(context, map)
    }
    ...
    fun autoCapitalize(ic: InputConnection?, info: EditorInfo?): Boolean {
        ...
        if (type and InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS != 0 || type and InputType.TYPE_TEXT_FLAG_CAP_WORDS != 0) return true
        if (type and InputType.TYPE_TEXT_FLAG_CAP_SENTENCES == 0) return false
        val before = ic.getTextBeforeCursor(80, 0)?.toString().orEmpty().trimEnd(); return before.isEmpty() || before.lastOrNull() in setOf('.', '!', '?', ':', ';', '\n')
    }
