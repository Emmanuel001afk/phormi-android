...
object PhormiKeyboardTextEngine {
    ...
    fun localeFor(info: EditorInfo?): Locale {
        val hinted = runCatching { info?.hintLocales?.get(0) }.getOrNull()
        return hinted ?: Locale.getDefault()
    }
    ...
    fun suggestions(context: Context, prefix: String, locale: Locale = Locale.getDefault()): List<String> {
        val p = prefix.lowercase(locale); if (p.isBlank()) return emptyList(); val french = locale.language == Locale.FRENCH; val pool = LinkedHashSet<String>()
        if (french) frenchCorrections[p]?.let(pool::add) else corrections[p]?.let(pool::add)
        loadLearned(context).filter { it.startsWith(p, ignoreCase = true) }.forEach(pool::add)
        (if (french) frenchWords else commonWords).filter { it.startsWith(p, ignoreCase = true) }.forEach(pool::add)
    ...
    fun nextWordSuggestions(context: Context, previous: String, locale: Locale = Locale.getDefault()): List<String> {
        val key = previous.lowercase(locale).trim(); if (key.isBlank()) return emptyList(); val pool = LinkedHashSet<String>(); (if (locale.language == Locale.FRENCH) frenchNextWordSeed[key] else nextWordSeed[key]).orEmpty().forEach(pool::add); loadNextWords(context, key).forEach(pool::add); return pool.take(4)
    }

    fun correctionFor(context: Context, word: String, locale: Locale = Locale.getDefault()): String? {
        val key = word.lowercase(locale)
        if (locale.language == Locale.FRENCH) return frenchCorrections[key] ?: loadLearnedCorrections(context)[key]
        return corrections[key] ?: loadLearnedCorrections(context)[key]
    }
    ...
    fun learn(context: Context, word: String, info: EditorInfo?) {
        if (word.isBlank() || isPrivateEditor(info)) return
        val normalized = word.trim(); if (normalized.length !in 2..48 || normalized.any { it.isDigit() }) return
        val words = loadLearned(context).toMutableList(); words.removeAll { it.equals(normalized, ignoreCase = true) }; words.add(0, normalized); saveLearned(context, words)
    }
    fun learnPair(context: Context, previous: String, current: String, info: EditorInfo?) {
        if (isPrivateEditor(info) || previous.isBlank() || current.isBlank()) return
        val key = previous.lowercase(Locale.getDefault()).take(48); val value = current.take(48); val all = loadAllNextWords(context).toMutableMap(); val values = (all[key].orEmpty().filterNot { it.equals(value, ignoreCase = true) } + value).takeLast(8); all[key] = values; saveAllNextWords(context, all)
    }
    fun learnCorrection(context: Context, original: String, accepted: String, info: EditorInfo?) {
        if (isPrivateEditor(info)) return
        val raw = original.trim(); val fixed = accepted.trim(); if (raw.length !in 2..48 || fixed.isBlank() || raw.equals(fixed, ignoreCase = true)) return
        val map = loadLearnedCorrections(context).toMutableMap(); map[raw.lowercase(Locale.getDefault())] = fixed; saveLearnedCorrections(context, map)
    }

    fun autoCapitalize(ic: InputConnection?, info: EditorInfo?): Boolean {
        if (ic == null || info == null || isPrivateEditor(info)) return false; val flags = info.inputType; if (flags and InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS != 0 || flags and InputType.TYPE_TEXT_FLAG_CAP_WORDS != 0) return true; if (flags and InputType.TYPE_TEXT_FLAG_CAP_SENTENCES == 0) return false; val trimmed = ic.getTextBeforeCursor(64, 0)?.toString().orEmpty().trimEnd(); return trimmed.isEmpty() || trimmed.lastOrNull() in setOf('.', '!', '?', ':', ';', '\n')
    }
