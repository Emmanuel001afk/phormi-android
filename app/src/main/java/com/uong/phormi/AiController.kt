...
class AiController(private val context: Context) {
    ...
    companion object {
        ...
        val TEMPLATES = listOf(
            ...
            Provider("openai", "OpenAI", "https://api.openai.com/v1/chat/completions", "gpt-4o-mini", ""),
            Provider("gemini", "Gemini", "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", "gemini-1.5-flash", "")
        )
    }
    ...
    fun upsertProvider(provider: Provider) {
        ...
        val idx = list.indexOfFirst { it.id == provider.id || it.name.equals(provider.name, true) }
    ...
    suspend fun runTask(instruction: String, onStatus: (String) -> Unit) {
        ...
        if (!hasAnyKey()) {
            onStatus("Save at least one AI provider (name + key + endpoint) first.")
            return
        }
    ...
    private suspend fun callProvider(
        ...
        client.newCall(request).execute().use { response ->
            if (!response.isSuccessful) {
                Log.w(TAG, "${provider.name} HTTP ${response.code}")
