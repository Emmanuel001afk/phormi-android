...
req("DownloadsActivity.kt",["STATUS_RUNNING","formatProgress(","COLUMN_BYTES_DOWNLOADED_SO_FAR","PhormiFileOpener.open","cancelDownload"])
req("GhostActivity.kt",["WebView.setDataDirectorySuffix","FLAG_SECURE","finishAndClear","restoreTabs"])
...
req("TabGroupManager.kt",["assignTab","groupForTab","tabIds"]);req("TabGroupsActivity.kt",["readCurrentTabs","select_tab_id","Task note"]);req("BrowserLockManager.kt",["BiometricPrompt","DEVICE_CREDENTIAL","canUseDeviceAuthentication"]);req("PhormiFavorites.kt",["contains","toggle","getAll"])
try:
    mt=MENU.read_text()
